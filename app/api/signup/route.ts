...
export async function POST(req: NextRequest) {
  try {
    ...
    // Cek dulu env var wajib sebelum coba konek ke Supabase, biar errornya jelas
    // bukan bikin request menggantung tanpa keterangan.
    if (!process.env.SUPABASE_SERVICE_ROLE_KEY) {
      console.error("SUPABASE_SERVICE_ROLE_KEY belum di-set di environment variables.");
      return NextResponse.json(
        {
          error:
            "Konfigurasi server belum lengkap (SUPABASE_SERVICE_ROLE_KEY belum diisi). Hubungi admin.",
        },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json(
      { error: "Terjadi kesalahan tak terduga di server. Coba lagi." },
      { status: 500 }
    );
  }
}
